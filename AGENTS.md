...
|---|---|---|---|
| `config.py` | ~40 | All crawl settings in one dataclass | `CrawlConfig(start_url, max_pages, max_depth, num_workers, delay_seconds, timeout_seconds, max_retries, retry_backoff_seconds, same_domain_only, include_subdomains, respect_robots, check_orphans, render_js, follow_redirects, user_agent, exclude_patterns, seed_urls, extract_rules)`, `.validate()` |
...
| `parser.py` | ~90 | BeautifulSoup audit extraction, handles `<base href>` | `parse_page(base_url, html, extract_rules=()) -> ParsedPage`; `parse_extract_rules(specs)` validates 'name=css:sel'/'name=re:pat' specs (ValueError) (title, links, meta_description, canonical_url, meta_robots, h1_count, image_count, images_missing_alt, missing_alt_srcs, mixed_content, word_count, content_hash; word count/hash exclude script/style; mixed_content only populated for https pages) |
...
| `jsfetch.py` | ~120 | Optional JS rendering (playwright, `fetchly[js]` extra). Dedicated render thread owns the sync Playwright API (it is greenlet/thread-bound); workers post to a queue. Same fetch() interface as Fetcher; has a plain `.session` for robots/sitemap | `JsFetcher(config)` — RuntimeError with install hint if playwright missing |
| `compare.py` | ~75 | `fetchly-compare` entry point: diff two page CSVs | `load_report(path)`, `diff_reports(old, new) -> {added, removed, changed}`; tracks status/title/meta/canonical/redirect/word_count |
| `viz.py` | ~110 | Self-contained HTML link-graph (inline JS canvas force layout, no external resources) | `write_graph(path, results) -> node_count`; edges from found_on |
| `gui/app.py` | ~290 | Tkinter front end (`fetchly-gui` entry point) | `FetchlyApp(root)`, `main()`. Settings form, Pages/Issues notebook tabs, progress bar, Stop, Export (writes both CSVs) |
